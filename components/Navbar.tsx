  ...
  return (
    ...
    >
      <div className="max-w-7xl mx-auto px-6 flex justify-between items-center">
        ...
        >
          <div className={`w-8 h-8 rounded-lg flex items-center justify-center transition-colors ${isScrolled ? 'bg-itot-teal text-white' : 'bg-white text-itot-teal'}`}>
             <svg viewBox="0 0 109.39 122.88" className="w-5 h-5 fill-current">
        ...
        <div className="lg:hidden absolute top-full left-0 right-0 bg-white border-t border-slate-100 shadow-2xl py-6 px-6 flex flex-col gap-6 animate-in slide-in-from-top-4 duration-300">
          {navLinks.map((item) => (
            <NavLink 
              ...
              onClick={handleLinkClick} 
              className={({ isActive }) => `
                text-lg font-bold border-b border-slate-50 pb-3 px-4 rounded-xl flex justify-between items-center transition-all
                ${isActive ? 'text-itot-teal bg-teal-50' : 'text-slate-800'}
              `}
            >
              {({ isActive }) => (
                <>
                  {item.label}
                  {isActive && <div className="w-2 h-2 rounded-full bg-itot-teal shadow-lg shadow-teal-200"></div>}
                </>
              )}
            </NavLink>
          ))}
